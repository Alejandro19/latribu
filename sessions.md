...
## 2026-07-22 (cont.) — Audio propio en Cortisol/Descanso, validación obligatoria del onboarding, login con Google

**Fix de puerto local**: LATRIBU y BIO360 competían por el puerto 3000 al dejar ambos corriendo en background. Se fijó `PORT=3001` en el `.env` de LATRIBU (`.env`/`.env.example`) para que cada proyecto tenga su propio puerto fijo y nunca vuelvan a chocar.

**Editar tips de Cortisol sin recargar**: se reemplazó el botón "Desactivar" por "Editar" en el banco de tips ("Sabías que...") de Gestión de Cortisol, con el mismo patrón de edición en línea + refresco dirigido ya usado en Frases y Herramientas para dormir.

**Información Personal (admin) — vista "sin cliente" corregida dos veces**: primero se hizo que mostrara la card completa del wizard (con el buscador embebido) en vez de un placeholder, igual que los demás módulos; después se corrigió que las filas de campos (Fecha de nacimiento, Género, etc.) se ocultaban si no había datos — ahora siempre muestran la etiqueta con "—" en vez de desaparecer, para que la estructura completa del formulario sea visible desde el primer momento.

**Audio propio en "Herramientas para dormir" (Descanso)**: además del cronómetro ya existente, el admin ahora puede subir un archivo de audio real por herramienta (`rest_tools.audio_url`/`audio_name`, subida vía `POST /api/admin/rest-tools/:id/upload-audio` a Supabase Storage). Se agregó también duración en minutos **y segundos** por separado (`seconds` además de `minutes`), ya que muchos audios no duran un número redondo de minutos. Se quitó el botón "Desactivar" del listado (solo Editar/Eliminar), y tanto crear como editar suben el audio automáticamente al guardar (no hace falta un paso aparte). El botón "Reproducir" del cliente ahora reproduce el audio real con controles nativos (pausa/adelantar/retroceder) en vez de solo mostrar un número bajando.

**Mismo tratamiento de audio en Gestión de Cortisol**: "Asignar técnica" (y su edición) ahora incluye Duración en min:seg y carga de audio propio (`cortisol_techniques.audio_url`/`audio_name`/`duration_minutes`/`duration_seconds`, endpoint `upload-audio` propio). El reproductor del cliente muestra el audio con controles nativos cuando la técnica no tiene video/YouTube asociado. En la lista "Tus técnicas" se corrigió que el botón "Reproducir" no aparecía para técnicas que solo tenían audio (sin video) — ahora si es audio-only, el reproductor se despliega en línea, justo debajo de esa fila, en vez de navegar a otra pantalla.

**Limpieza de archivos huérfanos en Supabase Storage**: se detectó que reemplazar o eliminar un audio dejaba el archivo anterior huérfano en el bucket (un caso real ya había acumulado ~45MB del mismo archivo subido 3 veces). Se agregó `deleteOldStorageFile()` en `server.js`, que borra el archivo previo del storage al reemplazar audio, al hacer "Quitar audio", o al eliminar la herramienta/técnica completa. Se limpiaron los duplicados ya existentes.

**Restricción de descarga por rol**: se acordó que solo el admin puede descargar contenido; el cliente solo carga y visualiza. Se añadió `controlsList="nodownload"` + bloqueo de clic derecho a todos los reproductores de audio/video que ve el cliente (Cortisol, Descanso, Entrenamiento), condicionado a `!isAdmin`. Los botones de descarga de PDF del cliente en Nutrición se habían quitado por error en una primera pasada y se restauraron a pedido explícito del usuario — quedan disponibles tanto para "Descargar plan (PDF)" como "Descargar PDF" de suplementos.

**Deep link al check-in desde el hero de Descanso**: el botón "Hacer tu check-in de hoy" ahora navega a Mi Evolución con la card "Check-in rápido del mes" ya abierta y hace scroll automático hasta ella (antes solo cambiaba de módulo, dejando al cliente en la parte de arriba). Se corrigió un bug donde el scroll ocurría antes de que la gráfica de evolución terminara de renderizarse, lo que desplazaba la card fuera de la vista por el cambio de altura de la página — se movió el scroll a después del render de la gráfica.

**Validación obligatoria en los 9 módulos de Información Personal**: los módulos 2 y 4-9 ya tenían `required: true` declarado en cada campo, pero se confirmó que el enforcement (`validateStep`, con borde rojo en los campos vacíos) ya estaba activo. Los módulos 1 (Perfil Personal) y 3 (Composición Corporal) no tenían ninguna validación real — `saveModule1()`/`saveModule3()` avanzaban sin verificar nada. Se agregó `validateModule1()` (exige país, ciudad y celular además de los campos genéricos) y `validateModule3()` (exige peso, estatura, % grasa y todos los campos del InBody), **excluyendo explícitamente "Ángulo de fase" y toda la sección "Medidas antropométricas"**, que quedan opcionales por decisión del usuario. Si el campo faltante está dentro de un acordeón colapsado, la sección se abre automáticamente para que se vea el error.

**Login con Google (Google Identity Services)**: se agregó un botón "Continuar con Google" (tema oscuro, `filled_black`) en la pantalla de login, sin cambiar el diseño existente. Backend: nuevo endpoint `POST /api/auth/google` que verifica el ID token con `google-auth-library` contra el `GOOGLE_CLIENT_ID` real (creado por el usuario en Google Cloud Console), y `GET /api/config` para exponerlo al frontend sin hardcodearlo en `index.html`. Reglas acordadas explícitamente con el usuario: login con Google disponible tanto para clientes como para admin (match por email verificado); si el email no coincide con ninguna cuenta existente, se crea un cliente nuevo **inactivo**, igual que el registro manual, pendiente de aprobación del admin. Se agregó `google_id` a `clients` y `admins` (solo como referencia de vinculación, no se usa para autenticar) y se hizo `clients.password_hash` nullable (las cuentas creadas por Google no tienen contraseña). Se corrigió un bug real: el popup de Google quedaba en blanco porque `helmet` pone por defecto `Cross-Origin-Opener-Policy: same-origin`, lo que bloquea la comunicación del popup con la página — se cambió a `same-origin-allow-popups`, el fix estándar documentado para este caso.

Todo verificado en vivo contra Supabase real con datos desechables (crear → verificar → eliminar) antes de dar cualquier cambio por confirmado.

## 2026-07-23 — Deploy en Vercel, responsive móvil, y hero de sueño rediseñado en Descanso

**Fix de despliegue en Vercel (dos bugs reales encontrados en producción)**: el sitio daba 404 incluso en la URL única de cada deployment "Ready". Causa: `vercel.json` usaba el formato legacy `builds`/`routes`, que solo declaraba `api/server.js` como salida de build — `index.html` nunca quedaba incluido en el deploy. Se cambió a `rewrites` (formato zero-config), dejando que Vercel detecte la función y sirva los estáticos automáticamente. Con eso resuelto, apareció un segundo error real: `ERR_REQUIRE_ESM` porque `uuid` v14 (instalado vía `^14.0.1`) dejó de tener build CommonJS — se reemplazó por `crypto.randomUUID()` nativo de Node, quitando la dependencia por completo.

**Responsive completo del shell de la app**: el sidebar (250px fijo) no tenía ningún ajuste para pantallas chicas — en celular se veía aplastado junto al contenido, con texto de una sola palabra por línea. Se agregó un menú hamburguesa/panel deslizable por debajo de 900px de ancho (con fondo oscuro detrás, se cierra solo al elegir un módulo), se redujo el padding y el tamaño de encabezados en pantallas medianas/chicas, las tablas anchas ahora se desplazan horizontalmente dentro de su card en vez de aplastar columnas, y las filas de listas permiten bajar de línea en vez de encimarse.

**Ajustes de UX puntuales**: se quitó la frase en cursiva redundante del wizard de Información Personal ("Conocerte es el primer paso..."), y se eliminó el texto "Cargando..." que aparecía brevemente al cambiar de módulo (ahora el contenido anterior se queda visible hasta que el nuevo módulo reemplaza el DOM directamente, sin flash de texto).

**Caché de peticiones GET (20s) para navegación casi instantánea**: se agregó una caché en memoria dentro de la función `api()` compartida por las ~90 llamadas de toda la app — volver a un módulo visto hace poco carga desde memoria en vez de esperar otro viaje de red. Cualquier mutación (POST/PUT/PATCH/DELETE) limpia toda la caché automáticamente para no arriesgar datos desactualizados tras editar algo; también se limpia en `logout()` para no filtrar datos entre sesiones del mismo navegador. No elimina los "cold starts" de Vercel (propios del hosting serverless gratuito), pero sí toda la navegación repetida dentro de una sesión activa.

**Hero de sueño de Descanso, rediseñado por completo (ejecución del prompt `descanso_final`)**: reemplazó el hero anterior, que leía `sleep_hours` del check-in mensual de Mi Evolución. Ahora es un registro diario propio, de un toque, sin salir de la pantalla: stepper de horas (incrementos de 0.5h) + selector de calidad (5 caras) + botón Guardar, todo dentro del hero. Al guardar, cambia automáticamente al estado "Ya registrado" mostrando las horas, la calidad, y "te dormiste alrededor de las HH:MM" (calculado restando las horas dormidas al momento exacto del guardado — no es un dato inventado), con link para editar el registro de hoy. Nueva tabla `sleep_logs` (una fila por cliente por día, upsert sobre `client_id`+`date` — reemplazar el registro de "hoy" no crea filas duplicadas). Se eliminaron `goToCheckinToday()`/`openCheckinAccordionIfRequested()` y su deep-link a Mi Evolución, que quedaron huérfanos al reemplazar el botón que los usaba.

Todo verificado en vivo: el fix de Vercel se probó viendo los logs reales de deploy/función hasta confirmar 200 en vez de 404/500; el hero de sueño se probó contra Supabase real (crear registro → verificar cálculo de hora de dormir → editar mismo día vía upsert → confirmar → limpiar).

## 2026-07-23 (cont.) — Confirmación de asistencia vía NFC + racha semanal + protector (Entrenamiento)

Ejecución del prompt `nfc_racha_completo`: función central única `confirmarSesionEntrenamiento(source)` (`'manual'`/`'nfc'`, solo para analítica) que llaman tanto el botón existente "Marcar entrenamiento del día como completado" como el flujo de escaneo NFC — nunca lógica duplicada.

**Decisión de diseño explícita, distinta al pseudocódigo del prompt**: en vez de contadores mutables (`streakWeeks`, `sessionsDoneThisWeek`) guardados como campos en `clients`, todo se calcula al vuelo a partir de filas reales — reutilizando `training_completions` (ya existía) más una tabla nueva `training_protector_uses`, en vez de crear una tabla de "asistencia" separada. Nunca se puede desincronizar, a costa de recalcularse en cada request. `training_completions` ganó una columna `source` (`'manual'`/`'nfc'`) solo para analítica.

**Link NFC**: `https://dominio/c?m=entrenamiento&a=confirmar&t={token}` tal cual especificado — el token identifica el sticker/ubicación, no a un cliente (el cliente sale de la sesión autenticada). Se agregó la ruta `/c` a `vercel.json` (necesaria porque la SPA no tiene un catch-all de rutas después del fix de deploy de ayer). Sesión no autenticada: la acción pendiente se guarda en `localStorage` (en vez del redirect a `/login?pendingAction=true` del pseudocódigo, ya que esto es un SPA de una sola página) y se ejecuta sola justo después de loguearse, vía `consumePendingActionIfAny()` enganchado dentro de `boot()` — el cliente nunca tiene que volver a escanear ni buscar un botón.

**Protector de racha**: 1 disponible por semana calendario (lunes-domingo), no acumulable. Usarlo hace que la semana cuente como completada para la racha aunque falten sesiones — visualmente pinta todos los puntos restantes de esa semana como 🛡️ protegidos (un solo uso real, aunque se vean varios escudos — confirmado con el usuario que es el comportamiento esperado, igual que en el mockup de referencia).

**Extensibilidad**: mapa `DEEP_LINK_ACTIONS` (`'entrenamiento:confirmar'` por ahora) — agregar NFC a Cortisol o Comunidad más adelante es una entrada nueva ahí, no una reconstrucción.

Verificado en vivo contra Supabase real con un cliente activo real (racha en 0 → confirmar sube a 1/4 sesiones → confirmar de nuevo el mismo día es idempotente, no duplica fila → usar protector sube la racha a 1 inmediatamente → usar protector dos veces no duplica fila), limpiando después solo las filas de prueba y dejando intacto el historial real del cliente.
