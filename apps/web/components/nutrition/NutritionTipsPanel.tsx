...
import { listTips, createTip, deleteTip, updateTip, type NutritionTip } from '../../lib/nutrition-tips-client';
...
export function NutritionTipsPanel() {
  ...
  function startEdit(tip: NutritionTip) {
    setEditingId(tip.id);
  ...
  return (
    <div>
      <label style={labelStyle} htmlFor="ntp-new-content">Nuevo tip</label>
      <textarea
        id="ntp-new-content"
        rows={2}
        style={textareaStyle}
        ...
        placeholder="Ej: prepara tus comidas con anticipación para evitar decisiones de último momento."
      />
      ...
      <div style={{ marginTop: 16 }}>
        {tips.length === 0 ? (
          <EmptyState message="Aún no hay tips de nutrición." />
        ) : (
          tips.map((tip) =>
            editingId === tip.id ? (
